...
func testDriverContexts(t *testing.T, task *structs.Task) *testContext {
	...
	tmpdrv, err := driver.NewDriver(task.Driver, driver.NewEmptyDriverContext())
	if err != nil {
		allocDir.Destroy()
	...
	execCtx := driver.NewExecContext(td, eb.Build())

	logger := testlog.Logger(t)
	emitter := func(m string, args ...interface{}) {
		logger.Printf("[EVENT] "+m, args...)
	}
	driverCtx := driver.NewDriverContext(alloc.Job.Name, alloc.TaskGroup, task.Name, alloc.ID, cfg, cfg.Node, logger, emitter)
...
func setupTaskEnv(t *testing.T, driv string) (*allocdir.TaskDir, map[string]string, map[string]string) {
	task := &structs.Task{
		Name:   "Foo",
		Driver: driv,
	...
	tmpDriver, err := driver.NewDriver(driv, driver.NewEmptyDriverContext())
	if err != nil {
		t.Fatalf("unable to create driver %q: %v", driv, err)
	}
